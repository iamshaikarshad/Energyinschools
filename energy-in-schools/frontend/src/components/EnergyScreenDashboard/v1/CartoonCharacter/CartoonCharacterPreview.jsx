...
const CartoonCharacterPreview = ({ classes, energyMood }) => {
  let title,
    status,
    picture = emptyPlaceholder;
  const pickedType = Object.keys(energyMood)[0];

  if (pickedType) {
    title = pickedType;
    status = MOOD_VALUE_TITLE_MAPPING_UNIVERSAL[energyMood[pickedType]];
    picture = MOOD_VALUE_PICTURE_MAPPING[pickedType][energyMood[pickedType]][0];
