...
import number4Img from '../../../../images/4.svg';
...
class EnergyFactsListingOld extends React.Component {
  constructor(props) {
    super(props);
    this.state = { factsToDisplay: [] };
  }

  componentWillMount() {
    const { facts } = this.props;
    const factsToDisplay = shuffle(facts).slice(0, MAX_FACTS_ON_DASHBOARD);
    this.setState({ factsToDisplay }); // eslint-disable-line react/no-did-update-set-state
  }

  render() {
    const { classes } = this.props;
    const { factsToDisplay } = this.state;
    const numbersImages = [number1Img, number2Img, number3Img, number4Img];

    const factsComponents = factsToDisplay.map((fact, index) => (
      <EnergyFactCard key={fact.id} factText={truncateText(fact.text, MAX_NEWS_SYMBOLS)} indexNum={index + 1} indexNumImg={numbersImages[index]} />
    ));
...
EnergyFactsListingOld.propTypes = {
  classes: PropTypes.object.isRequired,
  facts: PropTypes.array,
};

EnergyFactsListingOld.defaultProps = {
  facts: { text: '' },
};

export default compose(withStyles(styles))(EnergyFactsListingOld);
